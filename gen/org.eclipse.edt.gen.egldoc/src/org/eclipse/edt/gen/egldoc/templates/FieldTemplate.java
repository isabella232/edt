	...
	public void genDeclaration(Field field, Context ctx, TabbedWriter out) {
		...
		if (!theList.isEmpty()) {
			...
			while (iterator.hasNext()){
				...
				/*
				 * Instance of: eglx.lang.ExternalName eClass -> 
				 * Instance of: org.eclipse.edt.mof.egl.AnnotationType 
				 * - eglx.lang.ExternalName annotations 
				 * -> [] metadata -> [] value -> hello 
				 * Instance of: EGL_Location eClass 
				 * -> Instance of: org.eclipse.edt.mof.EClass 
				 * - EGL_Location annotations 
				 * -> [] metadata -> [] len -> 41 off -> 182 line -> 18 
				 */
				
				/*
				 * List fields  = annotation.getEClass().getEFields();
				 * 
				...
				// "eglx.lang.ExternalName EGL_Location":  out.println(iterator.next().getEClass().getETypeSignature());
				annotationName = iterator.next().getEClass().getETypeSignature();
				
				// display, element, and list (if appropriate); but assume no list
				stringDetails = (ArrayList<String>) Util.getEGLSimpleType(annotationName);
				out.println("<a href=\"" + stringDetails.get(1) + "\">");
				out.println(stringDetails.get(0) + "</a>");
				
				// "hello 41": out.println(iterator.next().getValue().toString());			
