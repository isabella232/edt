...
public class Util {
	...
	public static String findBlockComment(Context ctx, Integer endingOnLine){
		...
		DefaultLineTracker tracker = (DefaultLineTracker)ctx.get("lineTracker");
		...
		for(Iterator<Symbol> it = blockComments.iterator(); it.hasNext();)
		{
			...
			int commentEndLine;
			try {
				commentEndLine = tracker.getLineNumberOfOffset(blockComment.right);
				if((endingOnLine-2) == commentEndLine){
					result = fileContents.substring(blockComment.left, blockComment.right);
				}
	...
	private static Map<String, String> buildCommentMap(String[] sectionsFromCommentBlock) {
	...
	private static String[] extractSectionsFromCommentBlock(String[] linesFromCommentBlock) {
	...
	private static String[] extractLinesFromCommentBlock(String commentBlock) {
		...
		for (int i = 0; i < lines.length; i++) {
			if(lines.length > 0){
				if(i == 0){
					// This is the first line in the comment block - remove the leading /**, any additional *'s, and any white space
