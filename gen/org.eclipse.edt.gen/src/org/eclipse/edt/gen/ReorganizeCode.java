...
public class ReorganizeCode extends AbstractVisitor {
	...
	public boolean visit(ForStatement object) {
		...
		if (counterVariableHasSideEffects || fromExpressionHasSideEffects || toExpressionHasSideEffects || deltaExpressionHasSideEffects) {
			...
			LocalVariableDeclarationStatement localDeclarationFlag = factory.createLocalVariableDeclarationStatement();
			if (object.getAnnotation(IEGLConstants.EGL_LOCATION) != null)
				localDeclarationFlag.addAnnotation(object.getAnnotation(IEGLConstants.EGL_LOCATION));
			localDeclarationFlag.setContainer(currentStatementContainer);
			DeclarationExpression declarationExpressionFlag = factory.createDeclarationExpression();
			if (object.getAnnotation(IEGLConstants.EGL_LOCATION) != null)
				declarationExpressionFlag.addAnnotation(object.getAnnotation(IEGLConstants.EGL_LOCATION));
			Field fieldFlag = factory.createField();
			fieldFlag.setName(temporaryFlag);
			fieldFlag.setType(TypeUtils.Type_BOOLEAN);
			// we need to create the member access
			MemberName nameExpressionFlag = factory.createMemberName();
			if (object.getAnnotation(IEGLConstants.EGL_LOCATION) != null)
				nameExpressionFlag.addAnnotation(object.getAnnotation(IEGLConstants.EGL_LOCATION));
			nameExpressionFlag.setMember(fieldFlag);
			nameExpressionFlag.setId(fieldFlag.getName());
			// add the field to the declaration expression
			declarationExpressionFlag.getFields().add(fieldFlag);
			// connect the declaration expression to the local declaration
			...
			ifStatement.setCondition(nameExpressionFlag);
			// create the true and false statement blocks
			StatementBlock trueStatementBlock = factory.createStatementBlock();
			trueStatementBlock.setContainer(ifStatement.getContainer());
			StatementBlock falseStatementBlock = factory.createStatementBlock();
			falseStatementBlock.setContainer(ifStatement.getContainer());
			...
			AssignmentStatement assignmentStatementFlag = factory.createAssignmentStatement();
			if (object.getAnnotation(IEGLConstants.EGL_LOCATION) != null)
				assignmentStatementFlag.addAnnotation(object.getAnnotation(IEGLConstants.EGL_LOCATION));
			assignmentStatementFlag.setContainer(currentStatementContainer);
			Assignment assignmentFlag = factory.createAssignment();
			if (object.getAnnotation(IEGLConstants.EGL_LOCATION) != null)
				assignmentFlag.addAnnotation(object.getAnnotation(IEGLConstants.EGL_LOCATION));
			assignmentStatementFlag.setAssignment(assignmentFlag);
			assignmentFlag.setLHS(nameExpressionFlag);
			assignmentFlag.setRHS(booleanLiteral);
			// add the assignment statement to the false statement block
			falseStatementBlock.getStatements().add(assignmentStatementFlag);
			// add the statement block to the true condition
			ifStatement.setTrueBranch(trueStatementBlock);
			...
			if (fromExpressionHasSideEffects) {
				...
				Field field = factory.createField();
				field.setName(temporary);
				field.setType(object.getFromExpression().getType());
				field.setIsNullable(object.getFromExpression().isNullable());
				// we need to create the member access
				MemberName nameExpression = factory.createMemberName();
				if (object.getAnnotation(IEGLConstants.EGL_LOCATION) != null)
					nameExpression.addAnnotation(object.getAnnotation(IEGLConstants.EGL_LOCATION));
				...
				assignmentStatement.setAssignment(assignment);
				assignment.setLHS(nameExpression);
				assignment.setRHS(object.getFromExpression());
				// add the assignment to the declaration statement block
				StatementBlock declarationBlock = factory.createStatementBlock();
				declarationBlock.setContainer(currentStatementContainer);
				declarationBlock.getStatements().add(assignmentStatement);
			...
			// do we need to create a temporary variable for the delta expression
			if (deltaExpressionHasSideEffects) {
				// we need to create a local variable
				String temporary = ctx.nextTempName();
				LocalVariableDeclarationStatement localDeclaration = factory.createLocalVariableDeclarationStatement();
				if (object.getAnnotation(IEGLConstants.EGL_LOCATION) != null)
				...
				Field field = factory.createField();
				field.setName(temporary);
				field.setType(object.getDeltaExpression().getType());
				field.setIsNullable(object.getDeltaExpression().isNullable());
				// we need to create the member access
				MemberName nameExpression = factory.createMemberName();
				if (object.getAnnotation(IEGLConstants.EGL_LOCATION) != null)
					nameExpression.addAnnotation(object.getAnnotation(IEGLConstants.EGL_LOCATION));
				...
				if (object.getAnnotation(IEGLConstants.EGL_LOCATION) != null)
					assignmentStatementDelta.addAnnotation(object.getAnnotation(IEGLConstants.EGL_LOCATION));
				assignmentStatementDelta.setContainer(currentStatementContainer);
				Assignment assignmentDelta = factory.createAssignment();
				if (object.getAnnotation(IEGLConstants.EGL_LOCATION) != null)
					assignmentDelta.addAnnotation(object.getAnnotation(IEGLConstants.EGL_LOCATION));
				assignmentStatementDelta.setAssignment(assignmentDelta);
				assignmentDelta.setLHS(nameExpression);
				assignmentDelta.setRHS(object.getDeltaExpression());
				// add the assignment to the start of the user's statement block, after our if statement
				((StatementBlock) object.getBody()).getStatements().add(1, assignmentStatementDelta);
				// now replace the condition with the temporary variable
				object.setDeltaExpression(nameExpression);
			}
			// do we need to create a temporary variable for the to expression
			if (toExpressionHasSideEffects) {
				// we need to create a local variable
				String temporary = ctx.nextTempName();
				LocalVariableDeclarationStatement localDeclaration = factory.createLocalVariableDeclarationStatement();
				if (object.getAnnotation(IEGLConstants.EGL_LOCATION) != null)
				...
				Field field = factory.createField();
				field.setName(temporary);
				field.setType(object.getToExpression().getType());
				field.setIsNullable(object.getToExpression().isNullable());
				// we need to create the member access
				MemberName nameExpression = factory.createMemberName();
				if (object.getAnnotation(IEGLConstants.EGL_LOCATION) != null)
					nameExpression.addAnnotation(object.getAnnotation(IEGLConstants.EGL_LOCATION));
				...
				assignmentStatement.setAssignment(assignment);
				assignment.setLHS(nameExpression);
				assignment.setRHS(object.getToExpression());
				// add the assignment to the declaration statement block
				StatementBlock declarationBlock = factory.createStatementBlock();
				declarationBlock.setContainer(currentStatementContainer);
				declarationBlock.getStatements().add(assignmentStatement);
				...
				assignmentStatementTrue.setAssignment(assignmentTrue);
				assignmentTrue.setLHS(nameExpression);
				assignmentTrue.setRHS((Expression) object.getToExpression().clone());
				// add the assignment to the true statement block
				trueStatementBlock.getStatements().add(assignmentStatementTrue);
				// now replace the condition with the temporary variable
				object.setToExpression(nameExpression);
			}
		}
		// if the statement has an exit or continue, then we need to set a flag to indicate that a label is needed
		ReorganizeLabel reorganizeLabel = new ReorganizeLabel();
