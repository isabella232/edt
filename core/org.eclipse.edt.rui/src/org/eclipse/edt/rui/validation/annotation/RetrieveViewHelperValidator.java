...
public class RetrieveViewHelperValidator implements IValueValidationRule {
	...
	public void validate(Node errorNode, Node target, Annotation annotation, IProblemRequestor problemRequestor, ICompilerOptions compilerOptions) {
		Object value = annotation.getValue(NameUtile.getAsName(IEGLConstants.PROPERTY_RETRIEVEVIEWHELPER));
		if (value instanceof FunctionMember) {
			FunctionMember function = (FunctionMember)value;
			if (!isValidRetrieveViewHelperFunction(function)) {
				problemRequestor.acceptProblem(
						errorNode,
						RUIResourceKeys.RETRIEVEVEVIEWHELPER_FUNCTION_INVALID,
	...
	public boolean isValidRetrieveViewHelperFunction(FunctionMember function) {
