 ...
 *
 */
public class PropertyEditorInteger extends PropertyEditorAbstract implements FocusListener, SelectionListener {
	...
	public void initialize() {
		...
		if( strValue.length() > 0 )
			_text.setSelection( strValue.length() );
	}
	...
	public void focusGained( FocusEvent e ) {
		...
		if( strText.length() > 0 )
			_text.setSelection( strText.length() );
	}
	...
	/**
	 * Notifies the superclass of the value change. 
	 */
	protected void valueChanged(){
		...
		if( _text.getText().length() > 0 ) {
			try {
				new Integer( _text.getText() );
				_text.setBackground( null );
			}
			catch( NumberFormatException ex ) {
				_text.setBackground( _text.getDisplay().getSystemColor( SWT.COLOR_YELLOW ) );
				return;
			}
		}
