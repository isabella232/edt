...
public abstract class AbstractTargetHandle implements ITargetHandle {
	 ...
	 * based targets.
	 */
	public static final IPath BUNDLE_POOL = EDTCoreIDEPlugin.getPlugin().getStateLocation().append(".bundle_pool"); //$NON-NLS-1$

	/**
	 * Path to the local directory where install folders are created for p2 profile
	 * based targets.
	 */
	static final IPath INSTALL_FOLDERS = EDTCoreIDEPlugin.getPlugin().getStateLocation().append(".install_folders"); //$NON-NLS-1$	
	 ...
	 */
	static final String PROP_INSTALLED_IU = EGLCore.PLUGIN_ID + ".installed_iu"; //$NON-NLS-1$

	/**
	 * Profile property that keeps track of provisioning mode for the target
	 * (slice versus plan).
	 */
	static final String PROP_PROVISION_MODE = EGLCore.PLUGIN_ID + ".provision_mode"; //$NON-NLS-1$

	/**
	 * Profile property that keeps track of provisioning mode for the target
	 * (all environments/true versus false).
	 */
	static final String PROP_ALL_ENVIRONMENTS = EGLCore.PLUGIN_ID + ".all_environments"; //$NON-NLS-1$	
