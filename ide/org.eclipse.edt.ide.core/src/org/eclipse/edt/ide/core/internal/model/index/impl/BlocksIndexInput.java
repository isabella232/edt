...
import org.eclipse.edt.ide.core.internal.model.util.HashtableOfInt;
	...
	public void open() throws IOException {
		if (!opened) {
			...
			if (!sig.equals(IIndexConstants.SIGNATURE))
				throw new IOException(EGLModelSearchResources.ExceptionWrongFormat);
		...
		while(blockLoc >= 0){
			...
			while (block.nextEntry(entry)) {
				if (CharOperation.prefixEquals(prefix, entry.getWord()) ||
					CharOperation.prefixEquals(org.eclipse.edt.ide.core.model.IIndexConstants.REF,entry.getWord())) {
					if (count == entries.length){
						System.arraycopy(entries, 0, entries = new IEntryResult[count*2], 0, count);
					}
					entries[count++] = new EntryResult(entry.getWord(), entry.getRefs());
