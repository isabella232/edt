...
public void save(IProgressMonitor progress, boolean force) throws EGLModelException {
	...
	synchronized (this.lock) {
		...
		try {
			String encoding = this.file.getCharset();
			if(encoding == null)	//get the default encoding setting
				encoding = ((IEGLElement)this.owner).getEGLProject().getOption(EGLCore.CORE_ENCODING, true);			
...
public String toString() {
	...
	buffer.append("Owner: " + ((EGLElement)this.owner).toStringWithAncestors()); //$NON-NLS-1$
