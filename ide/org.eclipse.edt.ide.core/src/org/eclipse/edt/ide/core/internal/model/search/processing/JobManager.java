					...
					try {
						...
						while ((awaitingWork = awaitingJobsCount()) > 0) {
							...
							if (currentJob != null && currentJob != previousJob) {
								...
								if (subProgress != null) {
									subProgress.subTask(
										EGLModelSearchResources.bind(EGLModelSearchResources.ManagerFilesToIndex, Integer.toString(awaitingWork))); //$NON-NLS-1$
	...
	public void run() {
		...
		try {
			...
			if (this.thread != null) { // if not shutting down
				// log exception
				org.eclipse.edt.ide.core.internal.model.Util.log(e, "Background Indexer Crash Recovery"); //$NON-NLS-1$
