...
/**
 * Common protocol for EGL elements that support working copies.
 * <p>
 * A working copy of a EGL element acts just like a regular element (handle),
 * except it is not attached to an underlying resource. A working copy is not
 * visible to the rest of the EGL model. Changes in a working copy's
 * buffer are not realized in a resource. To bring the EGL model up-to-date with a working
 ...
 * destroying the working copy. The EGL model will never automatically
 ...
 * <code>EGLModelException</code> indicating the EGL element does not exist.
 * </p>
 * <p>
...
public interface IWorkingCopy {
	 ...
	 * and underlying resource, bringing the EGL model up-to-date with
	 ...
	 * @exception EGLModelException if this working copy could not commit. Reasons include:
	 * <ul>
	 ...
	 * </ul>
	 */
	void commit(boolean force, IProgressMonitor monitor) throws EGLModelException;
	 ...
	 * for this working copy will result in <code>IEGLModelException</code>s. Has
	 ...
	 * </p><p>
	 * When it is destroyed, a REMOVED IEGLElementDelta is reported on this 
	 * working copy.
	 * </p>
	 */
	void destroy();
	 ...
	 * @since 2.0
	 */
	IEGLElement findSharedWorkingCopy(IBufferFactory bufferFactory);
	 ...
	 */
	IEGLElement getOriginal(IEGLElement workingCopyElement);
	...
	/** 
	 * Finds the elements in this egl file that correspond to
	 * the given element.
	 ...
	 *     their respective egl files.
	 * <li>A exists.
	 * </ul>
	 * Returns <code>null</code> if no such java elements can be found
	 * or if the given element is not included in a egl file.
	 * 
	 * @param element the given element
	 * @return the found elements in this egl file that correspond to the given element
	 * @since 2.0 
	 */
	IEGLElement[] findElements(IEGLElement element);
	
	/**
	 * Finds the primary type of this egl file (that is, the type with the same name as the
	 * egl file), or <code>null</code> if no such a type exists.
	 * 
	 * @return the found primary type of this egl file, or <code>null</code> if no such a type exists
	 * @since 2.0
	 */
	 ...
	 * original egl file AND was using the same buffer factory (that is, as defined by <code>Object.equals</code>).	 
	 * <p>
	 ...
	 * The buffer will be automatically initialized with the original's egl file content
	 * upon creation.
	 * <p>
	 * When the shared working copy instance is created, an ADDED IEGLElementDelta is reported on this
	 * working copy.
	 *
	 * @param monitor a progress monitor used to report progress while opening this egl file
	 ...
	 * @exception EGLModelException if the contents of this element can
	 *   not be determined. 
	 ...
	 * @since 2.0
	 */
	IEGLElement getSharedWorkingCopy(
	 ...
	 * </p><p>
	 * When the working copy instance is created, an ADDED IEGLElementDelta is 
	 ...
	 *
	 * @exception EGLModelException if the contents of the original element
	 *		cannot be accessed. Reasons include:
	 * <ul>
	 * <li> The original EGL element does not exist (ELEMENT_DOES_NOT_EXIST)</li>
	 * </ul>
	 * @return <code>null</code>
	 */
	 ...
	 * @since 2.0
	 */
	void reconcile(boolean forceProblemDetection, IProgressMonitor monitor) throws EGLModelException;
