...
public class Resources {
	...
	private static IStatus addModified(IStatus status, IFile file) {
		IStatus entry= EGLUIStatus.createError(
			IEGLStatusConstants.VALIDATE_EDIT_CHANGED_CONTENT,
			Messages.format(ResourcesMessages.Resources_fileModified, BasicElementLabels.getPathLabel(file.getFullPath(), false)),
			null);
		if (status == null) {
			return entry;
			...
			MultiStatus result= new MultiStatus(EDTUIPlugin.getPluginId(),
				IEGLStatusConstants.VALIDATE_EDIT_CHANGED_CONTENT,
				ResourcesMessages.Resources_modifiedResources, null);
			result.add(status);
			result.add(entry);
			return result;
		}
	...
	private static IStatus addOutOfSync(IStatus status, IResource resource) {
		IStatus entry= new Status(
			...
			Messages.format(ResourcesMessages.Resources_outOfSync, BasicElementLabels.getPathLabel(resource.getFullPath(), false)),
			null);
		if (status == null) {
			return entry;
		} else if (status.isMultiStatus()) {
			...
			MultiStatus result= new MultiStatus(
				ResourcesPlugin.PI_RESOURCES,
				IResourceStatus.OUT_OF_SYNC_LOCAL,
				ResourcesMessages.Resources_outOfSyncResources, null);
			result.add(status);
			result.add(entry);
			return result;
		}
