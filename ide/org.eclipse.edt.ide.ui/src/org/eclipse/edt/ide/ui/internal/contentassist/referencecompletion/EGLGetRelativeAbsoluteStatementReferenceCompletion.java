...
public class EGLGetRelativeAbsoluteStatementReferenceCompletion extends EGLAbstractReferenceCompletion {
		...
		getBoundASTNodeForOffsetInStatement(viewer, documentOffset, new IBoundNodeProcessor() {public void processBoundNode(Node boundNode) {
			...
			proposals.addAll(
				new EGLDeclarationProposalHandler(viewer,
					...
					prefix,
					boundNode)
						.getDataItemProposals(EGLDeclarationProposalHandler.INTEGER_DATAITEM, true, getParens()));
			...
			proposals.addAll(
				new EGLDeclarationProposalHandler(viewer,
					...
					prefix,
					boundNode)
						.getRecordProposals(EGLDeclarationProposalHandler.ALL_RECORDS, false, false, getParens()));
		}});
