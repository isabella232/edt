...
public class EGLCorrectionAssistant extends QuickAssistAssistant {
	...
	public static int collectQuickFixableAnnotations(ITextEditor editor, int invocationLocation, boolean goToClosest, ArrayList resultingAnnotations) throws BadLocationException {
		IAnnotationModel model= EDTUIPlugin.getDefault().getEGLDocumentProvider().getAnnotationModel(editor.getEditorInput());
		if (model == null) {
		...
		if (goToClosest) {
			...
			while (iter.hasNext()) {
				...
				if ( EGLCorrectionProcessor.isQuickFixableType(annot)) {
		...
		if (isInside(invocationLocation, posBegin, posEnd)) {
			...
			if (newClosestPosition != -1) {
				if (newClosestPosition != bestOffset) { 
					if (EGLCorrectionProcessor.hasCorrections(annot)) {
