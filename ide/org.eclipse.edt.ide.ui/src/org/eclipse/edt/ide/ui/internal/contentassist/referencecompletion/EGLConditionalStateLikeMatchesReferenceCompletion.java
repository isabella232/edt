...
public class EGLConditionalStateLikeMatchesReferenceCompletion extends EGLAbstractReferenceCompletion {
	...
	protected void precompileContexts() {
		addContext("package a; function a() if (a like"); //$NON-NLS-1$
		addContext("package a; function a() if (a matches"); //$NON-NLS-1$
		addContext("package a; function a() if (a matches b escape"); //$NON-NLS-1$
	}
	...
	protected List returnCompletionProposals(ParseStack parseStack, final String prefix, final ITextViewer viewer, final int documentOffset) {
		...
		getBoundASTNodeForOffsetInStatement(viewer, documentOffset, new IBoundNodeProcessor() {public void processBoundNode(Node boundNode) {
			...
			//Get all dataItem variable proposals
			proposals.addAll(
				new EGLDeclarationProposalHandler(viewer,
					documentOffset,
					prefix,
