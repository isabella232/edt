...
//		PlatformUI.getWorkbench().getHelpSystem().setHelp(newShell, IJavaHelpContextIds.VARIABLE_CREATION_DIALOG);
	...
	private String getInitPath() {
		...
		if (initPath.length() == 0) {		
			initPath= fDialogSettings.get(IEGLUIConstants.DIALOGSTORE_LASTEXTEGLAR);
			if (initPath == null) {
				initPath= ""; //$NON-NLS-1$
			}
		} else {
	...
	private IPath chooseExtJarFile() {
		...
		if (res != null) {
			fDialogSettings.put(IEGLUIConstants.DIALOGSTORE_LASTEXTEGLAR, dialog.getFilterPath());
