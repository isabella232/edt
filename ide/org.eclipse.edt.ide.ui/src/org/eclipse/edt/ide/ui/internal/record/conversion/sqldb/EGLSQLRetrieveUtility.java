...
public class EGLSQLRetrieveUtility {
	...
	public boolean resolveToEGLType(DatabaseDefinition databaseDefinition, PredefinedDataType type, String tableName, String itemName, EGLSQLStructureItem sqlStructureItem, boolean allowTextTypesForDateTimeTypes, boolean isNullable) {
		...
		/**
		 * RATLC01174618 Guy Slade
		 * NUMERIC used to be treated as Decimal. The perfomance fix
		 * changes it to be handled by an EGL NUM type. This should only be a path 
		 * for iSeries Zoned Numeric data types.
		 */
		case Types.NUMERIC: {
		...
		case Types.CLOB: {
			...
			//if (getEGLBasePlugin().getCobolCompatibleRecordOption()) {
				//addInfoMessage(EGLMessage.EGLMESSAGE_INFO_UNSUPPORTED_TYPE_FOR_COBOL_ON_RETRIEVE, new String[]{tableName, type.getName()}); // TODO
			//}
			break;
		}
		case Types.BLOB: {
	...
	public boolean populateStructureItem(DatabaseDefinition definition, Column column, EGLSQLStructureItem item) {
		...
		//	item.getMessages().add(getInfoMessage(
			...
			if (resolveToEGLType(definition, type, column.getTable().getName(), itemName, item, false, column.isNullable())) {
				...
				//item.getMessages().add(getInfoMessage(
					//	EGLMessage.EGLMESSAGE_INFO_UNSUPPORTED_SQL_TYPE_ON_RETRIEVE,
						//new String[] { itemName, type.getName() })); // TODO
			}
		}
			...
			try {
				...
				/*newItem.getMessages().add(
								getErrorMessage(
										...
										new String[] { stringLength, itemName }));*/ // TODO:
...
/*			newItem.getMessages().add(
							getErrorMessage(
									...
									new String[] { stringDecimal, itemName }));*/ // TODO:
		...
		if (length > 32) {
			/*newItem.getMessages().add(
							getInfoMessage(
									...
									new String[] { itemName, stringLength }));*/ // TODO:
		...
		if (decimals > 32) {
			/*newItem.getMessages().add(
							getInfoMessage(
									...
									new String[] { itemName, stringDecimal }));*/ // TODO:
