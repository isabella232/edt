...
//	private SelectionButtonDialogField fExternalFolderButton;
	...
	public SourceAttachmentBlock(IStatusChangeListener context, IEGLPathEntry entry) {
		...
		Assert.isTrue(kind == IEGLPathEntry.CPE_LIBRARY || kind == IEGLPathEntry.CPE_VARIABLE);
...
//			fExternalFolderButton.setLabelText(NewWizardMessages.SourceAttachmentBlock_filename_externalfolder_button); 
	...
	private boolean isVariableEntry() {
		return fEntry.getEntryKind() == IEGLPathEntry.CPE_VARIABLE;
	}
	...
	public IEGLPathEntry getNewEntry() {
		PPListElement elem= PPListElement.createFromExisting(fEntry, fProject);
		elem.setAttribute(PPListElement.SOURCEATTACHMENT, getSourceAttachmentPath());
...
////			return;
	...
	private IPath getResolvedPath(IPath path) {
		if (path != null) {
			...
			if (varName != null) {
				IPath varPath= EGLCore.getEGLPathVariable(varName);
				if (varPath != null) {
	...
	private IStatus updateFileNameStatus() {
		...
		if (fileName.length() == 0) {
			...
			if (isVariableEntry()) {
				...
				fFileVariablePath= EGLCore.getEGLPathVariable(varName);
				if (fFileVariablePath == null) {
	...
	private IPath chooseExtension() {
		...
		EGLARFileSelectionDialog dialog= new EGLARFileSelectionDialog(getShell(), false, true, false);
...
//	 */
//	public static IRunnableWithProgress getRunnable(final Shell shell, final IEGLPathEntry newEntry, final IEGLProject jproject, final IPath containerPath) {
