...
final class ContentAssistAdvancedConfigurationBlock extends OptionsConfigurationBlock {

	private static final Key PREF_EXCLUDED_CATEGORIES= getEGLUIKey(EDTUIPreferenceConstants.CODEASSIST_EXCLUDED_CATEGORIES);
	private static final Key PREF_CATEGORY_ORDER= getEGLUIKey(EDTUIPreferenceConstants.CODEASSIST_CATEGORY_ORDER);
	...
	private final class PreferenceModel {
		...
		public PreferenceModel(EGLCompletionProposalComputerRegistry registry) {
    	...
    	private boolean readInclusionPreference(EGLCompletionProposalCategory cat) {
    	...
    	private int readOrderPreference(EGLCompletionProposalCategory cat) {
	...
	private final class ModelElement {
		...
		ModelElement(EGLCompletionProposalCategory category, PreferenceModel model) {
			fCategory= category;
			ICommandService commandSvc= (ICommandService) PlatformUI.getWorkbench().getAdapter(ICommandService.class);
			fCommand= commandSvc.getCommand("org.eclipse.edt.ide.ui.specific_content_assist.command"); //$NON-NLS-1$
			IParameter type;
			try {
		...
		Image getImage() {
			return ContentAssistAdvancedConfigurationBlock.this.getImage(fCategory.getImageDescriptor());
		}
		String getName() {
	...
	ContentAssistAdvancedConfigurationBlock(IStatusChangeListener statusListener, IWorkbenchPreferenceContainer container) {
		super(statusListener, null, getAllKeys(), container);
		fModel= new PreferenceModel(EGLCompletionProposalComputerRegistry.getDefault());
	}
