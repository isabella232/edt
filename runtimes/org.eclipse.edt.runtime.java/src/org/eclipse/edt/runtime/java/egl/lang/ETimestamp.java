...
/**
 * A class for Timestamps.  The value is a Calendar and a number of microseconds
 * (an int) which is encapsulated in a TimestampData object.  Timestamps store a
 * varying set of fields, from years down to microseconds.  The first and last 
 * fields are indicated by the startCode and endCode fields, which are set from
 * the "CODE" constants.
 * 
 * @author mheitz
 */
...
//		}
	...
	/**
	 * Converts a formatted string into a Calendar that's suitable for 
	 * assigning or comparing with this item.  Strings are parsed using
	 * defaultTimeStampFormat if it's not blank, otherwise we use the pattern
	 * field of this object.  If the string can't be parsed using a pattern then
	 * we do it by hand, pulling out digits and ignoring non-digit characters.
	 * <P>
	 * When we parse using defaultTimeStampFormat, only the fields from the format
	 * string are set.  The rest default to zeros so you get Jan 1 1970, etc.
	 * When we parse by hand, unspecified fields are set to the current time.
	 * This behavior is OK for assignments, but it must be accounted for when
	 * comparing two timestamps.
	 * 
	 * @param source
	 *            a string describing the new value.
		...
		throws JavartException
	{
		...
		try {
			result = convert(source, format);
		}
		catch (ParseException pe) {
			// OK. Do it ourselves
			result = convert(source, startCode, endCode);
		}
		return result;
	}
	
	/**
	 * Returns the value as a String. This is specifically for use in string
	 * concatenations. Some Values have other methods that return a String, and
	 * the results do not need to match.
	 * 
	 * @param program
	 *            The program
	...
	public static String asString( Executable program, Calendar value ) throws JavartException
	{
		...
		{
			JavartDateFormat formatter = 
				DateTimeUtil.getDateFormat( format );
	...
	private static Calendar convert(String source, int startCode, int endCode) {
		...
		if (length > 0) {
			...
			if (i <= length && startCode <= MINUTE_CODE
					&& endCode >= MINUTE_CODE) {
				minutes = ch - '0';
				if (i < length) {
					ch = source.charAt(i);
			...
			if (i <= length && startCode <= SECOND_CODE
					&& endCode >= SECOND_CODE) {
				seconds = ch - '0';
				if (i < length) {
					ch = source.charAt(i);
			...
			if (i <= length && endCode >= FRACTION1_CODE) {
				...
				int fractionDigits = endCode - FRACTION1_CODE + 1;
				while (microsecondsFound < fractionDigits && '0' <= ch
						&& ch <= '9') {
					microseconds *= 10;
					microseconds += ch - '0';
					microsecondsFound++;
		...
		if ((years == -1 && startCode == YEAR_CODE)
				|| (months == -1 && startCode <= MONTH_CODE && endCode >= MONTH_CODE)
				|| (days == -1 && startCode <= DAY_CODE && endCode >= DAY_CODE)
				|| (hours == -1 && startCode <= HOUR_CODE && endCode >= HOUR_CODE)
				|| (minutes == -1 && startCode <= MINUTE_CODE && endCode >= MINUTE_CODE)
				|| (seconds == -1 && startCode <= SECOND_CODE && endCode >= SECOND_CODE)
	...
	public static Calendar convert( String source, String format )
