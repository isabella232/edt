/* Generated from 'egl.javascript.BigDecimal.nrx' 8 Sep 2000 11:10:50 [v2.00] */
...
/* ------------------------------------------------------------------ */
/* egl.javascript.BigDecimal -- Decimal arithmetic for Java                          */
/* ------------------------------------------------------------------ */
/*                                                                    */
/* The egl.javascript.BigDecimal class provides immutable arbitrary-precision        */
/* floating point (including integer) decimal numbers.                */
/*                                                                    */
/* As the numbers are decimal, there is an exact correspondence       */
/* between an instance of a egl.javascript.BigDecimal object and its String          */
/* representation; the egl.javascript.BigDecimal class provides direct conversions   */
...
/* Notes:                                                             */
/*                                                                    */
/* 1. A egl.javascript.BigDecimal object is never changed in value once constructed; */
/*    this avoids the need for locking.  Note in particular that the  */
/*    mantissa array may be shared between many egl.javascript.BigDecimal objects,   */
/*    so that once exposed it must not be altered.                    */
/*                                                                    */
/* 2. This class looks at egl.javascript.MathContext class fields directly (for      */
...
/*    [This assertion depends on MAX_DIGITS in egl.javascript.MathContext.]          */
/*                                                                    */
...
/* 1998.06.04 rework to pass egl.javascript.MathContext to finish() and round()      */
/* 1998.06.06 change format to use round(); support rounding modes    */
/* 1998.06.25 rename to egl.javascript.BigDecimal and begin merge                    */
...
/* 1998.07.07 scaled division  [divide(egl.javascript.BigDecimal, int, int)]         */
...
/* 2000.06.18 1.08 no longer deprecate egl.javascript.BigDecimal(double)             */
/* ------------------------------------------------------------------ */
...
/* JavaScript conversion (c) 2003 STZ-IDA and PTV AG, Karlsruhe, Germany */
 ...
 /**
  * Constructs a <code>egl.javascript.BigDecimal</code> object from a
  * <code>java.math.egl.javascript.BigDecimal</code>.
  * <p>
  * Constructs a <code>egl.javascript.BigDecimal</code> as though the parameter had
  * been represented as a <code>String</code> (using its
  * <code>toString</code> method) and the
  * {@link #egl.javascript.BigDecimal(java.lang.String)} constructor had then been
  * used.
  * The parameter must not be <code>null</code>.
  * <p>
  * <i>(Note: this constructor is provided only in the
  * <code>com.ibm.icu.math</code> version of the egl.javascript.BigDecimal class.
  * It would not be present in a <code>java.math</code> version.)</i>
  *
  * @param bd The <code>egl.javascript.BigDecimal</code> to be translated.
  * @stable ICU 2.0
  */
 
 //--public egl.javascript.BigDecimal(java.math.egl.javascript.BigDecimal bd){
 //-- this(bd.toString());
 //-- return;}
  ...
  * <p>
  * Constructs a <code>egl.javascript.BigDecimal</code> which is the exact decimal
  * representation of the <code>BigInteger</code>, with a scale of
  * zero.
  * The value of the <code>egl.javascript.BigDecimal</code> is identical to the value
  * of the <code>BigInteger</code>.
  * The parameter must not be <code>null</code>.
  * <p>
  * The <code>egl.javascript.BigDecimal</code> will contain only decimal digits,
 ...
 //--public egl.javascript.BigDecimal(java.math.BigInteger bi){
 //-- this(bi.toString(10));
 //-- return;}
 // exp remains 0
  ...
  * The value of the <code>egl.javascript.BigDecimal</code> is the
  * <code>BigInteger</code> divided by ten to the power of the scale.
  * The <code>BigInteger</code> parameter must not be
  * <code>null</code>.
  * <p>
  * The <code>egl.javascript.BigDecimal</code> will contain only decimal digits, (with
 ...
 //--public egl.javascript.BigDecimal(java.math.BigInteger bi,int scale){
 //-- this(bi.toString(10));
 //-- if (scale<0) 
 ...
 /**
  * Constructs a <code>egl.javascript.BigDecimal</code> object from an array of characters.
  * <p>
  * Constructs a <code>egl.javascript.BigDecimal</code> as though a
  * <code>String</code> had been constructed from the character array
  * and the {@link #egl.javascript.BigDecimal(java.lang.String)} constructor had then
  * been used. The parameter must not be <code>null</code>.
  * <p>
  * Using this constructor is faster than using the
  * <code>egl.javascript.BigDecimal(String)</code> constructor if the string is
 ...
 //--public egl.javascript.BigDecimal(char inchars[]){
 ...
 //--public egl.javascript.BigDecimal(char inchars[],int offset,int length){super();
egl.javascript.BigDecimal = function cons() {
  ...
  if (arguments.length == 1)
   {
    inchars = arguments[0];
    if ( typeof(inchars) == 'number' )
    {
    	inchars = String(inchars);
    }
    else if ( inchars instanceof egl.javascript.BigDecimal )
    {
		this.ind = inchars.ind;
		this.form = inchars.form;
		this.mant = inchars.mant;
		this.exp = inchars.exp;
    	return;
    }
    offset = 0;
  ...
  if (length<=0) 
   this.bad("egl.javascript.BigDecimal(): ", inchars); // bad conversion (empty string)
  ...
  if (inchars[0]==('-')) 
   {
    length--;
    if (length==0) 
     this.bad("egl.javascript.BigDecimal(): ", inchars); // nothing after sign
    this.ind=this.isneg;
    offset++;
   }
  else 
  ...
  {var $1=length;i=offset;i:for(;$1>0;$1--,i++){
   ...
   if (si=='.') 
    { // record and ignore
     if (dotoff>=0) 
      this.bad("egl.javascript.BigDecimal(): ", inchars); // two dots
   ...
   if (si!='e') 
    if (si!='E') 
     { // expect an extra digit
      if (si<'0' || si>'9') 
       this.bad("egl.javascript.BigDecimal(): ", inchars); // not a number
   ...
   if ((i-offset)>(length-2)) 
    this.bad("egl.javascript.BigDecimal(): ", inchars); // no room for even one digit
   eneg=false;
   if ((inchars[i+1])==('-')) 
    {
     eneg=true;
   ...
   if ((elen==0)||(elen>9)) 
    this.bad("egl.javascript.BigDecimal(): ", inchars); // 0 or more than 9 digits
   {var $2=elen;j=k;j:for(;$2>0;$2--,j++){
    sj=inchars[j];
    if (sj<'0') 
     this.bad("egl.javascript.BigDecimal(): ", inchars); // always bad
    if (sj>'9') 
     { // maybe an exotic digit
      ...
      this.bad("egl.javascript.BigDecimal(): ", inchars);
     }
    else 
     dvalue=sj-'0';
  ...
  if (d==0) 
   this.bad("egl.javascript.BigDecimal(): ", inchars); // no mantissa digits
  if (dotoff>=0) 
    ...
    {var $4=d;i=0;i:for(;$4>0;$4--,i++){
     ...
     if (sj<='9') 
     ...
     else 
      {
       ...
       //if (dvalue<0) 
        this.bad("egl.javascript.BigDecimal(): ", inchars); // not a number after all
       //mant[i]=(byte)dvalue;
      }
     j++;
     }
 ...
 /**
  * Constructs a <code>egl.javascript.BigDecimal</code> object directly from a
  * <code>double</code>.
  * <p>
  * Constructs a <code>egl.javascript.BigDecimal</code> which is the exact decimal
  ...
  * and then using the {@link #egl.javascript.BigDecimal(java.lang.String)}
  * constructor.
  * To get that result, use the static {@link #valueOf(double)}
  * method to construct a <code>egl.javascript.BigDecimal</code> from a
  * <code>double</code>.
  *
 ...
 //--public egl.javascript.BigDecimal(double num){
 ...
 //-- this((new java.math.egl.javascript.BigDecimal(num)).toString());
 //-- return;}
 ...
 //--public egl.javascript.BigDecimal(int num){super();
 //-- int mun;
 //-- int i=0;
 ...
 //--public egl.javascript.BigDecimal(long num){super();
 //-- long mun;
 //-- int i=0;
 ...
 /**
  * Constructs a <code>egl.javascript.BigDecimal</code> object from a <code>String</code>.
  * <p>
  * Constructs a <code>egl.javascript.BigDecimal</code> from the parameter, which must
  * not be <code>null</code> and must represent a valid <i>number</i>,
  * as described formally in the documentation referred to
  * {@link egl.javascript.BigDecimal above}.
  * <p>
  ...
  * <p>
  * Some valid strings from which a <code>egl.javascript.BigDecimal</code> might
  * be constructed are:
  * <pre>
  *       "0"         -- Zero
  *      "12"         -- A whole number
  ...
  * <p>
  * The <code>egl.javascript.BigDecimal</code> constructed from the String is in a
  * standard form, with no blanks, as though the
  * {@link #add(egl.javascript.BigDecimal)} method had been used to add zero to the
  ...
  * <code>egl.javascript.BigDecimal</code> number will be expressed in scientific
 ...
 //--public egl.javascript.BigDecimal(java.lang.String string){
 //-- this(string.toCharArray(),0,string.length());
 //-- return;}

 /* <sgml> Make a default egl.javascript.BigDecimal object for local use. </sgml> */
 
 //--private egl.javascript.BigDecimal(){super();
 //-- return;
 //-- }
...
egl.javascript.BigDecimal.prototype.arraycopy = function(src, srcindex, dest, destindex, length) {
    var i;
...
egl.javascript.BigDecimal.prototype.createArrayWithZeros = function(length) {
...
/**
 * The <code>egl.javascript.BigDecimal</code> class implements immutable
 * arbitrary-precision decimal numbers.  The methods of the
 * <code>egl.javascript.BigDecimal</code> class provide operations for fixed and
 ...
 * an instance of a <code>egl.javascript.BigDecimal</code> object and its
 * <code>String</code> representation; the <code>egl.javascript.BigDecimal</code> class
 ...
 * the value of a <code>egl.javascript.BigDecimal</code> number object is shown as the
 ...
 * <p>
 * Operations on <code>egl.javascript.BigDecimal</code> numbers are controlled by a
 * {@link egl.javascript.MathContext} object, which provides the context (precision and
 * other information) for the operation. Methods that can take a
 * <code>egl.javascript.MathContext</code> parameter implement the standard arithmetic
 * operators for <code>egl.javascript.BigDecimal</code> objects and are known as
 * <i>operator methods</i>.  The default settings provided by the
 * constant {@link egl.javascript.MathContext#DEFAULT} (<code>digits=9,
 ...
 * precision.  The <code>egl.javascript.MathContext</code> parameter must not be
 * <code>null</code>.
 * <p>
 * Each operator method also has a version provided which does
 * not take a <code>egl.javascript.MathContext</code> parameter.  For this version of
 ...
 * precision, as defined for the original egl.javascript.BigDecimal class in Java 1.1
 * and Java 1.2.
 * <p>
 * For monadic operators, only the optional <code>egl.javascript.MathContext</code>
 * parameter is present; the operation acts upon the current object.
 * <p>
 * For dyadic operators, a <code>egl.javascript.BigDecimal</code> parameter is always
 * present; it must not be <code>null</code>.
 * The operation acts with the current object being the left-hand operand
 * and the <code>egl.javascript.BigDecimal</code> parameter being the right-hand operand.
 * <p>
 * For example, adding two <code>egl.javascript.BigDecimal</code> objects referred to
 * by the names <code>award</code> and <code>extra</code> could be
 * written as any of:
 * <p><code>
 *     award.add(extra)
 * <br>award.add(extra, egl.javascript.MathContext.DEFAULT)
 * <br>award.add(extra, acontext)
 * </code>
 * <p>
 * (where <code>acontext</code> is a <code>egl.javascript.MathContext</code> object),
 * which would return a <code>egl.javascript.BigDecimal</code> object whose value is
 * the result of adding <code>award</code> and <code>extra</code> under
 * the appropriate context settings.
 * <p>
 * When a <code>egl.javascript.BigDecimal</code> operator method is used, a set of
 * rules define what the result will be (and, by implication, how the
 * result would be represented as a character string).
 * These rules are defined in the egl.javascript.BigDecimal arithmetic documentation
 * (see the URL above), but in summary:
 * <ul>
 * <li>Results are normally calculated with up to some maximum number of
 * significant digits.
 * For example, if the <code>egl.javascript.MathContext</code> parameter for an operation
 * were <code>egl.javascript.MathContext.DEFAULT</code> then the result would be
 ...
 * method with a suitable <code>egl.javascript.MathContext</code> object. This lets you
 ...
 * <code>egl.javascript.MathContext</code> parameter).
 * <br>
 ...
 * So, for example:
 * <p><code>
 *     new egl.javascript.BigDecimal("2.40").add(     new egl.javascript.BigDecimal("2"))      =&gt; "4.40"
 * <br>new egl.javascript.BigDecimal("2.40").subtract(new egl.javascript.BigDecimal("2"))      =&gt; "0.40"
 * <br>new egl.javascript.BigDecimal("2.40").multiply(new egl.javascript.BigDecimal("2"))      =&gt; "4.80"
 * <br>new egl.javascript.BigDecimal("2.40").divide(  new egl.javascript.BigDecimal("2"), def) =&gt; "1.2"
 * </code>
 ...
 * <code>egl.javascript.MathContext.DEFAULT</code>).
 ...
 * <p><code>
 *   new egl.javascript.BigDecimal("1e+6").multiply(new egl.javascript.BigDecimal("1e+6"), def)
 * </code>
 * <p>results in <code>1E+12</code> instead of
 * <code>1000000000000</code>, and
 * <p><code>
 *   new egl.javascript.BigDecimal("1").divide(new egl.javascript.BigDecimal("3E+10"), def)
 * </code>
 ...
 * and <code>java.math.egl.javascript.BigDecimal</code> in Java 1.1 and Java 1.2.
 *
 * @see     egl.javascript.MathContext
 * @author  Mike Cowlishaw
 * @stable ICU 2.0
 */

//--public class egl.javascript.BigDecimal extends java.lang.Number implements java.io.Serializable,java.lang.Comparable{
//-- private static final java.lang.String $0="egl.javascript.BigDecimal.nrx";
  ...
  * @see egl.javascript.MathContext#ROUND_CEILING
  * @stable ICU 2.0
  */
 //--public static final int ROUND_CEILING=com.ibm.icu.math.egl.javascript.MathContext.ROUND_CEILING;
 egl.javascript.BigDecimal.prototype.ROUND_CEILING = egl.javascript.MathContext.prototype.ROUND_CEILING;
 
 /**
  * Rounding mode to round towards zero.
  * @see egl.javascript.MathContext#ROUND_DOWN
  * @stable ICU 2.0
  */
 //--public static final int ROUND_DOWN=com.ibm.icu.math.egl.javascript.MathContext.ROUND_DOWN;
 egl.javascript.BigDecimal.prototype.ROUND_DOWN = egl.javascript.MathContext.prototype.ROUND_DOWN;
 
 /**
  * Rounding mode to round to a more negative number.
  * @see egl.javascript.MathContext#ROUND_FLOOR
  * @stable ICU 2.0
  */
 //--public static final int ROUND_FLOOR=com.ibm.icu.math.egl.javascript.MathContext.ROUND_FLOOR;
 egl.javascript.BigDecimal.prototype.ROUND_FLOOR = egl.javascript.MathContext.prototype.ROUND_FLOOR;
 
 /**
  * Rounding mode to round to nearest neighbor, where an equidistant
  * value is rounded down.
  * @see egl.javascript.MathContext#ROUND_HALF_DOWN
  * @stable ICU 2.0
  */
 //--public static final int ROUND_HALF_DOWN=com.ibm.icu.math.egl.javascript.MathContext.ROUND_HALF_DOWN;
 egl.javascript.BigDecimal.prototype.ROUND_HALF_DOWN = egl.javascript.MathContext.prototype.ROUND_HALF_DOWN;
 
 /**
  * Rounding mode to round to nearest neighbor, where an equidistant
  * value is rounded to the nearest even neighbor.
  * @see egl.javascript.MathContext#ROUND_HALF_EVEN
  * @stable ICU 2.0
  */
 //--public static final int ROUND_HALF_EVEN=com.ibm.icu.math.egl.javascript.MathContext.ROUND_HALF_EVEN;
 egl.javascript.BigDecimal.prototype.ROUND_HALF_EVEN = egl.javascript.MathContext.prototype.ROUND_HALF_EVEN;
 
 /**
  * Rounding mode to round to nearest neighbor, where an equidistant
  * value is rounded up.
  * @see egl.javascript.MathContext#ROUND_HALF_UP
  * @stable ICU 2.0
  */
 //--public static final int ROUND_HALF_UP=com.ibm.icu.math.egl.javascript.MathContext.ROUND_HALF_UP;
 egl.javascript.BigDecimal.prototype.ROUND_HALF_UP = egl.javascript.MathContext.prototype.ROUND_HALF_UP;
 
 /**
  * Rounding mode to assert that no rounding is necessary.
  * @see egl.javascript.MathContext#ROUND_UNNECESSARY
  * @stable ICU 2.0
  */
 //--public static final int ROUND_UNNECESSARY=com.ibm.icu.math.egl.javascript.MathContext.ROUND_UNNECESSARY;
 egl.javascript.BigDecimal.prototype.ROUND_UNNECESSARY = egl.javascript.MathContext.prototype.ROUND_UNNECESSARY;
 
 /**
  * Rounding mode to round away from zero.
  * @see egl.javascript.MathContext#ROUND_UP
  * @stable ICU 2.0
  */
 //--public static final int ROUND_UP=com.ibm.icu.math.egl.javascript.MathContext.ROUND_UP;
 egl.javascript.BigDecimal.prototype.ROUND_UP = egl.javascript.MathContext.prototype.ROUND_UP;
 ...
 egl.javascript.BigDecimal.prototype.MinExp=-999999999; // minimum exponent allowed
 egl.javascript.BigDecimal.prototype.MaxExp=999999999; // maximum exponent allowed
 egl.javascript.BigDecimal.prototype.MinArg=-999999999; // minimum argument integer
 egl.javascript.BigDecimal.prototype.MaxArg=999999999; // maximum argument integer
 
 //--private static final com.ibm.icu.math.egl.javascript.MathContext plainMC=new com.ibm.icu.math.egl.javascript.MathContext(0,com.ibm.icu.math.egl.javascript.MathContext.PLAIN); // context for plain unlimited math
 egl.javascript.BigDecimal.prototype.plainMC=new egl.javascript.MathContext(0, egl.javascript.MathContext.prototype.PLAIN);
 egl.javascript.BigDecimal.prototype.eglMC=new egl.javascript.MathContext(32, egl.javascript.MathContext.prototype.PLAIN);
 ...
 /**
  * The <code>egl.javascript.BigDecimal</code> constant "0".
  *
  * @see #ONE
  * @see #TEN
  * @stable ICU 2.0
  */
 //--public static final com.ibm.icu.math.egl.javascript.BigDecimal ZERO=new com.ibm.icu.math.egl.javascript.BigDecimal((long)0); // use long as we want the int constructor
 // .. to be able to use this, for speed
egl.javascript.BigDecimal.prototype.ZERO = new egl.javascript.BigDecimal("0");

 /**
  * The <code>egl.javascript.BigDecimal</code> constant "1".
  *
  * @see #TEN
  * @see #ZERO
  * @stable ICU 2.0
  */
 //--public static final com.ibm.icu.math.egl.javascript.BigDecimal ONE=new com.ibm.icu.math.egl.javascript.BigDecimal((long)1); // use long as we want the int constructor
 // .. to be able to use this, for speed
egl.javascript.BigDecimal.prototype.ONE = new egl.javascript.BigDecimal("1");

 /**
  * The <code>egl.javascript.BigDecimal</code> constant "10".
  *
  * @see #ONE
  * @see #ZERO
  * @stable ICU 2.0
  */
 //--public static final com.ibm.icu.math.egl.javascript.BigDecimal TEN=new com.ibm.icu.math.egl.javascript.BigDecimal(10);
...
egl.javascript.BigDecimal.prototype.NINES = 
	[ new egl.javascript.BigDecimal("9"),
	  ...
	  new egl.javascript.BigDecimal("9999"), 
	  new egl.javascript.BigDecimal("99999"), 
	  new egl.javascript.BigDecimal("999999"), 
	  new egl.javascript.BigDecimal("9999999"), 
	  new egl.javascript.BigDecimal("99999999"), 
	  new egl.javascript.BigDecimal("999999999"), 
	  new egl.javascript.BigDecimal("9999999999"), 
	  new egl.javascript.BigDecimal("99999999999"), 
	  new egl.javascript.BigDecimal("999999999999"), 
	  new egl.javascript.BigDecimal("9999999999999"), 
	  new egl.javascript.BigDecimal("99999999999999"), 
	  new egl.javascript.BigDecimal("999999999999999"), 
	  new egl.javascript.BigDecimal("9999999999999999"), 
	  new egl.javascript.BigDecimal("99999999999999999"), 
	  new egl.javascript.BigDecimal("999999999999999999"), 
	  new egl.javascript.BigDecimal("9999999999999999999"), 
	  new egl.javascript.BigDecimal("99999999999999999999"), 
	  new egl.javascript.BigDecimal("999999999999999999999"), 
	  new egl.javascript.BigDecimal("9999999999999999999999"), 
	  new egl.javascript.BigDecimal("99999999999999999999999"), 
	  new egl.javascript.BigDecimal("999999999999999999999999"), 
	  new egl.javascript.BigDecimal("9999999999999999999999999"), 
	  new egl.javascript.BigDecimal("99999999999999999999999999"), 
	  new egl.javascript.BigDecimal("999999999999999999999999999"), 
	  new egl.javascript.BigDecimal("9999999999999999999999999999"), 
	  new egl.javascript.BigDecimal("99999999999999999999999999999"), 
	  new egl.javascript.BigDecimal("999999999999999999999999999999"), 
	  new egl.javascript.BigDecimal("9999999999999999999999999999999"), 
	  new egl.javascript.BigDecimal("99999999999999999999999999999999") ];
  ...
  * <ul>
  * <li>egl.javascript.MathContext.PLAIN        -- no exponent needed
  * <li>egl.javascript.MathContext.SCIENTIFIC   -- scientific notation required
  * <li>egl.javascript.MathContext.ENGINEERING  -- engineering notation required
  * </ul>
  * <p>
  ...
  * @serial
  */
 //--private byte form=(byte)com.ibm.icu.math.egl.javascript.MathContext.PLAIN; // assumed PLAIN
  ...
  * <p>
  * Once constructed, this may become shared between several egl.javascript.BigDecimal
 ...
 /**
  * Returns a plain <code>egl.javascript.BigDecimal</code> whose value is the absolute
  * value of this <code>egl.javascript.BigDecimal</code>.
  * <p>
  * The same as {@link #abs(egl.javascript.MathContext)}, where the context is
  * <code>new egl.javascript.MathContext(0, egl.javascript.MathContext.PLAIN)</code>.
  * <p>
  * The length of the decimal part (the scale) of the result will
  * be <code>this.scale()</code>
  *
  * @return A <code>egl.javascript.BigDecimal</code> whose value is the absolute
  *         value of this <code>egl.javascript.BigDecimal</code>.
  * @stable ICU 2.0
  */
 
 //--public com.ibm.icu.math.egl.javascript.BigDecimal abs(){
 //- return this.abs(plainMC);
 //- }

 /**
  * Returns a <code>egl.javascript.BigDecimal</code> whose value is the absolute value
  * of this <code>egl.javascript.BigDecimal</code>.
  * <p>
  * If the current object is zero or positive, then the same result as
  * invoking the {@link #plus(egl.javascript.MathContext)} method with the same
  * parameter is returned.
  * Otherwise, the same result as invoking the
  * {@link #negate(egl.javascript.MathContext)} method with the same parameter is
  * returned.
  *
  * @param  set The <code>egl.javascript.MathContext</code> arithmetic settings.
  * @return     A <code>egl.javascript.BigDecimal</code> whose value is the absolute
  *             value of this <code>egl.javascript.BigDecimal</code>.
  * @stable ICU 2.0
  */
 
 //--public com.ibm.icu.math.egl.javascript.BigDecimal abs(com.ibm.icu.math.egl.javascript.MathContext set){
 ...
 /**
  * Returns a plain <code>egl.javascript.BigDecimal</code> whose value is
  * <code>this+rhs</code>, using fixed point arithmetic.
  * <p>
  * The same as {@link #add(egl.javascript.BigDecimal, egl.javascript.MathContext)},
  * where the <code>egl.javascript.BigDecimal</code> is <code>rhs</code>,
  * and the context is <code>new egl.javascript.MathContext(0, egl.javascript.MathContext.PLAIN)</code>.
  * <p>
  * The length of the decimal part (the scale) of the result will be
  * the maximum of the scales of the two operands.
  *
  * @param  rhs The <code>egl.javascript.BigDecimal</code> for the right hand side of
  *             the addition.
  * @return     A <code>egl.javascript.BigDecimal</code> whose value is
  *             <code>this+rhs</code>, using fixed point arithmetic.
  * @stable ICU 2.0
  */
 
 //--public com.ibm.icu.math.egl.javascript.BigDecimal add(com.ibm.icu.math.egl.javascript.BigDecimal rhs){
 //-- return this.add(rhs,plainMC);
 //-- }

 /**
  * Returns a <code>egl.javascript.BigDecimal</code> whose value is <code>this+rhs</code>.
  * <p>
  * Implements the addition (<b><code>+</code></b>) operator
  * (as defined in the decimal documentation, see {@link egl.javascript.BigDecimal
  * class header}),
  * and returns the result as a <code>egl.javascript.BigDecimal</code> object.
  *
  * @param  rhs The <code>egl.javascript.BigDecimal</code> for the right hand side of
  *             the addition.
  * @param  set The <code>egl.javascript.MathContext</code> arithmetic settings.
 ...
 //--public com.ibm.icu.math.egl.javascript.BigDecimal add(com.ibm.icu.math.egl.javascript.BigDecimal rhs,com.ibm.icu.math.egl.javascript.MathContext set){
 egl.javascript.BigDecimal.prototype.add = function add() {
  ...
  res=new egl.javascript.BigDecimal(); // build result here
 ...
 /**
  * Compares this <code>egl.javascript.BigDecimal</code> to another, using unlimited
  * precision.
  * <p>
  * The same as {@link #compareTo(egl.javascript.BigDecimal, egl.javascript.MathContext)},
  * where the <code>egl.javascript.BigDecimal</code> is <code>rhs</code>,
  * and the context is <code>new egl.javascript.MathContext(0, egl.javascript.MathContext.PLAIN)</code>.
  *
  * @param  rhs The <code>egl.javascript.BigDecimal</code> for the right hand side of
 ...
 //--public int compareTo(com.ibm.icu.math.egl.javascript.BigDecimal rhs){
 //-- return this.compareTo(rhs,plainMC);
 //-- }

 /**
  * Compares this <code>egl.javascript.BigDecimal</code> to another.
  * <p>
 ...
 //public int compareTo(com.ibm.icu.math.egl.javascript.BigDecimal rhs,com.ibm.icu.math.egl.javascript.MathContext set){
  ...
  * <code>this/rhs</code>, using fixed point arithmetic.
  * <p>
  * The same as {@link #divide(egl.javascript.BigDecimal, int)},
  * where the <code>egl.javascript.BigDecimal</code> is <code>rhs</code>,
  * and the rounding mode is {@link egl.javascript.MathContext#ROUND_HALF_UP}.
  *
  * The length of the decimal part (the scale) of the result will be
  ...
  * @param  rhs The <code>egl.javascript.BigDecimal</code> for the right hand side of
  *             the division.
  * @return     A plain <code>egl.javascript.BigDecimal</code> whose value is
  *             <code>this/rhs</code>, using fixed point arithmetic.
  * @throws ArithmeticException if <code>rhs</code> is zero.
  * @stable ICU 2.0
  */
 
 //--public com.ibm.icu.math.egl.javascript.BigDecimal divide(com.ibm.icu.math.egl.javascript.BigDecimal rhs){
  ...
  * rounding mode.
  * <p>
  * The same as {@link #divide(egl.javascript.BigDecimal, int, int)},
  * where the <code>egl.javascript.BigDecimal</code> is <code>rhs</code>,
  ...
  *               the division (see the {@link egl.javascript.MathContext} class).
  * @return       A plain <code>egl.javascript.BigDecimal</code> whose value is
  *               <code>this/rhs</code>, using fixed point arithmetic
  ...
  *               egl.javascript.MathContext#ROUND_UNNECESSARY} and
  *               <code>this.scale()</code> is insufficient to
  *               represent the result exactly.
  * @stable ICU 2.0
  */
 
 //--public com.ibm.icu.math.egl.javascript.BigDecimal divide(com.ibm.icu.math.egl.javascript.BigDecimal rhs,int round){
 //-- com.ibm.icu.math.egl.javascript.MathContext set;
 //-- set=new com.ibm.icu.math.egl.javascript.MathContext(0,com.ibm.icu.math.egl.javascript.MathContext.PLAIN,false,round); // [checks round, too]
  ...
  * <p>
  * The same as {@link #divide(egl.javascript.BigDecimal, egl.javascript.MathContext)},
  * where the <code>egl.javascript.BigDecimal</code> is <code>rhs</code>,
  * <code>new egl.javascript.MathContext(0, egl.javascript.MathContext.PLAIN, false, round)</code>,
  ...
  * @throws ArithmeticException if <code>round</code> is {@link
  *               egl.javascript.MathContext#ROUND_UNNECESSARY} and <code>scale</code>
  *               is insufficient to represent the result exactly.
  * @stable ICU 2.0
  */
 
 //--public com.ibm.icu.math.egl.javascript.BigDecimal divide(com.ibm.icu.math.egl.javascript.BigDecimal rhs,int scale,int round){
 //-- com.ibm.icu.math.egl.javascript.MathContext set;
 //-- if (scale<0) 
 //--  throw new java.lang.ArithmeticException("Negative scale:"+" "+scale);
 //-- set=new com.ibm.icu.math.egl.javascript.MathContext(0,com.ibm.icu.math.egl.javascript.MathContext.PLAIN,false,round); // [checks round]
 ...
 //--public com.ibm.icu.math.egl.javascript.BigDecimal divide(com.ibm.icu.math.egl.javascript.BigDecimal rhs,com.ibm.icu.math.egl.javascript.MathContext set){
  ...
  if (arguments.length == 2)
   {
    if (typeof arguments[1] == 'number')
     {
      set=new egl.javascript.MathContext(0,egl.javascript.MathContext.prototype.PLAIN,false,arguments[1]); // [checks round, too]
     }
    else
     {
      set = arguments[1];
 ...
 /**
  * Returns a plain <code>egl.javascript.BigDecimal</code> whose value is the integer
  * part of <code>this/rhs</code>.
  * <p>
  * The same as {@link #divideInteger(egl.javascript.BigDecimal, egl.javascript.MathContext)},
  * where the <code>egl.javascript.BigDecimal</code> is <code>rhs</code>,
  * and the context is <code>new egl.javascript.MathContext(0, egl.javascript.MathContext.PLAIN)</code>.
  *
  * @param  rhs The <code>egl.javascript.BigDecimal</code> for the right hand side of
  *             the integer division.
  * @return     A <code>egl.javascript.BigDecimal</code> whose value is the integer
  *             part of <code>this/rhs</code>.
  * @throws ArithmeticException if <code>rhs</code> is zero.
  * @stable ICU 2.0
  */
 
 //--public com.ibm.icu.math.egl.javascript.BigDecimal divideInteger(com.ibm.icu.math.egl.javascript.BigDecimal rhs){
 //-- // scale 0 to drop .000 when plain
 //-- return this.dodivide('I',rhs,plainMC,0);
 //-- }

 /**
  * Returns a <code>egl.javascript.BigDecimal</code> whose value is the integer
  * part of <code>this/rhs</code>.
  * <p>
 ...
 //--public com.ibm.icu.math.egl.javascript.BigDecimal divideInteger(com.ibm.icu.math.egl.javascript.BigDecimal rhs,com.ibm.icu.math.egl.javascript.MathContext set){
  ...
  * <p>
  * The same as {@link #max(egl.javascript.BigDecimal, egl.javascript.MathContext)},
  * where the <code>egl.javascript.BigDecimal</code> is <code>rhs</code>,
  * and the context is <code>new egl.javascript.MathContext(0, egl.javascript.MathContext.PLAIN)</code>.
  *
  * @param  rhs The <code>egl.javascript.BigDecimal</code> for the right hand side of
 ...
 //--public com.ibm.icu.math.egl.javascript.BigDecimal max(com.ibm.icu.math.egl.javascript.BigDecimal rhs){
 //-- return this.max(rhs,plainMC);
 //-- }

 /**
  * Returns a <code>egl.javascript.BigDecimal</code> whose value is
  * the maximum of <code>this</code> and <code>rhs</code>.
  * <p>
  * Returns the larger of the current object and the first parameter.
  * <p>
  * If calling the {@link #compareTo(egl.javascript.BigDecimal, egl.javascript.MathContext)} method
  * with the same parameters would return <code>1</code> or
  * <code>0</code>, then the result of calling the
  * {@link #plus(egl.javascript.MathContext)} method on the current object (using the
  * same <code>egl.javascript.MathContext</code> parameter) is returned.
  * Otherwise, the result of calling the {@link #plus(egl.javascript.MathContext)}
  * method on the first parameter object (using the same
  * <code>egl.javascript.MathContext</code> parameter) is returned.
  *
  * @param  rhs The <code>egl.javascript.BigDecimal</code> for the right hand side of
  *             the comparison.
  * @param  set The <code>egl.javascript.MathContext</code> arithmetic settings.
 ...
 //--public com.ibm.icu.math.egl.javascript.BigDecimal max(com.ibm.icu.math.egl.javascript.BigDecimal rhs,com.ibm.icu.math.egl.javascript.MathContext set){
  ...
  * <p>
  * The same as {@link #min(egl.javascript.BigDecimal, egl.javascript.MathContext)},
  * where the <code>egl.javascript.BigDecimal</code> is <code>rhs</code>,
  * and the context is <code>new egl.javascript.MathContext(0, egl.javascript.MathContext.PLAIN)</code>.
  *
  * @param  rhs The <code>egl.javascript.BigDecimal</code> for the right hand side of
 ...
 //--public com.ibm.icu.math.egl.javascript.BigDecimal min(com.ibm.icu.math.egl.javascript.BigDecimal rhs){
 ...
 //--public com.ibm.icu.math.egl.javascript.BigDecimal min(com.ibm.icu.math.egl.javascript.BigDecimal rhs,com.ibm.icu.math.egl.javascript.MathContext set){
 ...
 //--public com.ibm.icu.math.egl.javascript.BigDecimal multiply(com.ibm.icu.math.egl.javascript.BigDecimal rhs){
 ...
 //--public com.ibm.icu.math.egl.javascript.BigDecimal multiply(com.ibm.icu.math.egl.javascript.BigDecimal rhs,com.ibm.icu.math.egl.javascript.MathContext set){
 egl.javascript.BigDecimal.prototype.multiply = function multiply() {
  ...
  res=new egl.javascript.BigDecimal(); // where we'll build result
  ...
  * <code>-this</code>.
  * <p>
  * The same as {@link #negate(egl.javascript.MathContext)}, where the context is
  * <code>new egl.javascript.MathContext(0, egl.javascript.MathContext.PLAIN)</code>.
  * <p>
  * The length of the decimal part (the scale) of the result will be
  * be <code>this.scale()</code>
 ...
 //--public com.ibm.icu.math.egl.javascript.BigDecimal negate(){
 //-- return this.negate(plainMC);
 //-- }

 /**
  * Returns a <code>egl.javascript.BigDecimal</code> whose value is <code>-this</code>.
  * <p>
 ...
 //public com.ibm.icu.math.egl.javascript.BigDecimal negate(com.ibm.icu.math.egl.javascript.MathContext set){
  ...
  * plain <code>egl.javascript.BigDecimal</code>, but the result will always be.
  * <p>
  * The same as {@link #plus(egl.javascript.MathContext)}, where the context is
  * <code>new egl.javascript.MathContext(0, egl.javascript.MathContext.PLAIN)</code>.
  * <p>
  * The length of the decimal part (the scale) of the result will be
  * be <code>this.scale()</code>
 ...
 //--public com.ibm.icu.math.egl.javascript.BigDecimal plus(){
 ...
 //public com.ibm.icu.math.egl.javascript.BigDecimal plus(com.ibm.icu.math.egl.javascript.MathContext set){
  ...
  * <code>this**rhs</code>, using fixed point arithmetic.
  * <p>
  * The same as {@link #pow(egl.javascript.BigDecimal, egl.javascript.MathContext)},
  * where the <code>egl.javascript.BigDecimal</code> is <code>rhs</code>,
  * and the context is <code>new egl.javascript.MathContext(0, egl.javascript.MathContext.PLAIN)</code>.
  * <p>
  ...
  * <code>egl.javascript.MathContext</code> is used and so the result would then
  * always be 0.
  *
  * @param  rhs The <code>egl.javascript.BigDecimal</code> for the right hand side of
 ...
 //--public com.ibm.icu.math.egl.javascript.BigDecimal pow(com.ibm.icu.math.egl.javascript.BigDecimal rhs){
  ...
  * <p>
  * If the <code>digits</code> setting of the <code>egl.javascript.MathContext</code>
 ...
 //--public com.ibm.icu.math.egl.javascript.BigDecimal pow(com.ibm.icu.math.egl.javascript.BigDecimal rhs,com.ibm.icu.math.egl.javascript.MathContext set){
 egl.javascript.BigDecimal.prototype.pow = function pow() {
  ...
  //--int L=0;
  var L=0;
  //--com.ibm.icu.math.egl.javascript.MathContext workset;
  var workset;
  //--com.ibm.icu.math.egl.javascript.BigDecimal res;
  var res;
  //--boolean seenbit;
  ...
  if (set.lostDigits) 
   this.checkdigits(rhs,set.digits);
  // if the rhs has non-zero decimal digits, use Math.pow to perform operation
  // and wrap it back into a BigDecimal Type
  if (rhs.exp<0 && !(rhs.allzero(rhs.mant,rhs.mant.length+rhs.exp))) 
  {
	  return new egl.javascript.BigDecimal(String(Math.pow(Number(this.toString()),Number(rhs.toString())))); 
  }
  ...
  workset=new egl.javascript.MathContext(workdigits,set.form,false,set.roundingMode);
...
//   }
//  }/*i*/ // 32 bits
// RES: 08-19-2008 - replaced above code with code that works :)
  for (var i=0;i<n;i++)
  {
  ...
  * <p>
  * The same as {@link #remainder(egl.javascript.BigDecimal, egl.javascript.MathContext)},
  * where the <code>egl.javascript.BigDecimal</code> is <code>rhs</code>,
  * and the context is <code>new egl.javascript.MathContext(0, egl.javascript.MathContext.PLAIN)</code>.
  * <p>
  ...
  * @return     A <code>egl.javascript.BigDecimal</code> whose value is the remainder
  *             of <code>this/rhs</code>, using fixed point arithmetic.
  * @throws ArithmeticException if <code>rhs</code> is zero.
  * @stable ICU 2.0
  */
 
 //--public com.ibm.icu.math.egl.javascript.BigDecimal remainder(com.ibm.icu.math.egl.javascript.BigDecimal rhs){
 //-- return this.dodivide('R',rhs,plainMC,-1);
 //-- }

 /**
  * Returns a <code>egl.javascript.BigDecimal</code> whose value is the remainder of
  * <code>this/rhs</code>.
  * <p>
 ...
 //--public com.ibm.icu.math.egl.javascript.BigDecimal remainder(com.ibm.icu.math.egl.javascript.BigDecimal rhs,com.ibm.icu.math.egl.javascript.MathContext set){
  ...
  * <code>this-rhs</code>, using fixed point arithmetic.
  * <p>
  * The same as {@link #subtract(egl.javascript.BigDecimal, egl.javascript.MathContext)},
  * where the <code>egl.javascript.BigDecimal</code> is <code>rhs</code>,
  * and the context is <code>new egl.javascript.MathContext(0, egl.javascript.MathContext.PLAIN)</code>.
  * <p>
  * The length of the decimal part (the scale) of the result will be
 ...
 //--public com.ibm.icu.math.egl.javascript.BigDecimal subtract(com.ibm.icu.math.egl.javascript.BigDecimal rhs){
 ...
 //--public com.ibm.icu.math.egl.javascript.BigDecimal subtract(com.ibm.icu.math.egl.javascript.BigDecimal rhs,com.ibm.icu.math.egl.javascript.MathContext set){
 ...
 /**
  * Converts this <code>egl.javascript.BigDecimal</code> to a <code>byte</code>.
  * If the <code>egl.javascript.BigDecimal</code> has a non-zero decimal part or is
 ...
 /**
  * Compares this <code>egl.javascript.BigDecimal</code> with the value of the parameter.
  * <p>
  * If the parameter is <code>null</code>, or is not an instance of the
  * <code>egl.javascript.BigDecimal</code> type, an exception is thrown.
  * Otherwise, the parameter is cast to type <code>egl.javascript.BigDecimal</code>
  * and the result of the {@link #compareTo(egl.javascript.BigDecimal)} method,
  * using the cast parameter, is returned.
  * <p>
  * The {@link #compareTo(egl.javascript.BigDecimal, egl.javascript.MathContext)} method should be
  * used when a <code>egl.javascript.MathContext</code> is needed for the comparison.
  *
  ...
  *                 a <code>egl.javascript.BigDecimal</code> object.
  * @see    #compareTo(egl.javascript.BigDecimal)
  * @stable ICU 2.0
  */
 
 //--public int compareTo(java.lang.Object rhsobj){
 //-- // the cast in the next line will raise ClassCastException if necessary
 //-- return compareTo((com.ibm.icu.math.egl.javascript.BigDecimal)rhsobj,plainMC);
 //-- }

 /**
  * Converts this <code>egl.javascript.BigDecimal</code> to a <code>double</code>.
  * If the <code>egl.javascript.BigDecimal</code> is out of the possible range for a
  ...
  * <code>egl.javascript.BigDecimal</code> as a <code>String</code> and then
 ...
 //-- // We go via a String [as does egl.javascript.BigDecimal in JDK 1.2]
 //-- // Next line could possibly raise NumberFormatException
 //-- return java.lang.Double.valueOf(this.toString()).doubleValue();
 //-- }

 /**
  * Compares this <code>egl.javascript.BigDecimal</code> with <code>rhs</code> for
  * equality.
  * <p>
  * If the parameter is <code>null</code>, or is not an instance of the
  * egl.javascript.BigDecimal type, or is not exactly equal to the current
  * <code>egl.javascript.BigDecimal</code> object, then <i>false</i> is returned.
  * Otherwise, <i>true</i> is returned.
  * <p>
  * "Exactly equal", here, means that the <code>String</code>
  * representations of the <code>egl.javascript.BigDecimal</code> numbers are
  ...
  * @see    #compareTo(Object)
  * @see    #compareTo(egl.javascript.BigDecimal)
  * @see    #compareTo(egl.javascript.BigDecimal, egl.javascript.MathContext)
  */
 
 //--public boolean equals(java.lang.Object obj){
 egl.javascript.BigDecimal.prototype.equals = function equals(obj) {
  //--com.ibm.icu.math.egl.javascript.BigDecimal rhs;
  var rhs;
  //--int i=0;
  var i=0;
  //--char lca[]=null;
 ...
 /**
  * Converts this <code>egl.javascript.BigDecimal</code> to a <code>float</code>.
  * If the <code>egl.javascript.BigDecimal</code> is out of the possible range for a
  ...
  * <code>egl.javascript.BigDecimal</code>, modified by layout parameters.
  * <p>
  ...
  * formatting that it provides is a necessary part of the egl.javascript.BigDecimal
  * class as it is sensitive to and must follow the calculation and
  * rounding rules for egl.javascript.BigDecimal arithmetic.
  ...
  * <code>egl.javascript.MathContext.ROUND_HALF_UP</code>.
  * <p>
  ...
  *                <code>egl.javascript.BigDecimal</code>, laid out according to the
 ...
 //-- return format(before,after,-1,-1,com.ibm.icu.math.egl.javascript.MathContext.SCIENTIFIC,ROUND_HALF_UP);
 //-- }

 /**
  * Returns the <code>String</code> representation of this
  * <code>egl.javascript.BigDecimal</code>, modified by layout parameters and allowing
  ...
  * needed).
  * It  may be either {@link egl.javascript.MathContext#SCIENTIFIC} or
  * {@link egl.javascript.MathContext#ENGINEERING}.
  ...
  * indicated by a public constant in the {@link egl.javascript.MathContext} class
  ...
  * <p>
  * The special value <code>egl.javascript.MathContext.ROUND_UNNECESSARY</code> may be
  ...
  *                  ({@link egl.javascript.MathContext#SCIENTIFIC} or
  *                  {@link egl.javascript.MathContext#ENGINEERING}).
  * @param  exround  The <code>int</code> specifying the rounding mode
  *                  to use.
  *                  Use -1 for the default, {@link egl.javascript.MathContext#ROUND_HALF_UP}.
  * @return          A <code>String</code> representing this
  *                  <code>egl.javascript.BigDecimal</code>, laid out according to the
  *                  specified parameters
  * @throws ArithmeticException if the number cannot be laid out as
 ...
 egl.javascript.BigDecimal.prototype.format = function format() {
  ...
  if (arguments.length == 6)
  ...
  else if (arguments.length == 2)
   {
    explaces = -1;
    exdigits = -1;
    exformint = egl.javascript.MathContext.prototype.SCIENTIFIC;
  ...
  //--com.ibm.icu.math.egl.javascript.BigDecimal num;
  var num;
  //--int mag=0;
  var mag=0;
  //--int thisafter=0;
  ...
  {/*select*/
  if (exformint==egl.javascript.MathContext.prototype.SCIENTIFIC)
   ;
  else if (exformint==egl.javascript.MathContext.prototype.ENGINEERING)
   ;
  else if (exformint==(-1))
   exformint=egl.javascript.MathContext.prototype.SCIENTIFIC;
  ...
  // egl.javascript.MathContext object with that mode; it will fail if bad
  if (exround!=this.ROUND_HALF_UP) 
   {try{ // if non-default...
    if (exround==(-1)) 
     exround=this.ROUND_HALF_UP;
    else 
     new egl.javascript.MathContext(9,egl.javascript.MathContext.prototype.SCIENTIFIC,false,exround);
   }
   catch ($10){
  ...
  /* Here:
     num       is egl.javascript.BigDecimal to format
  ...
  */
  ...
  if (after>=0) 
   {setafter:for(;;){
    ...
    else if (num.form==egl.javascript.MathContext.prototype.SCIENTIFIC)
 ...
 /**
  * Returns the hashcode for this <code>egl.javascript.BigDecimal</code>.
  * This hashcode is suitable for use by the
  * <code>java.util.Hashtable</code> class.
  * <p>
  * Note that two <code>egl.javascript.BigDecimal</code> objects are only guaranteed
  * to produce the same hashcode if they are exactly equal (that is,
  * the <code>String</code> representations of the
  * <code>egl.javascript.BigDecimal</code> numbers are identical -- they have the same
 ...
 /**
  * Converts this <code>egl.javascript.BigDecimal</code> to an <code>int</code>.
  * If the <code>egl.javascript.BigDecimal</code> has a non-zero decimal part it is
  * discarded. If the <code>egl.javascript.BigDecimal</code> is out of the possible
  ...
  if ((lodigit+useexp)==9) 
   {
    ...
    topdig=this.div(result,1000000000); // get top digit, preserving sign
 ...
 /**
  * Converts this <code>egl.javascript.BigDecimal</code> to a <code>long</code>.
  * If the <code>egl.javascript.BigDecimal</code> has a non-zero decimal part it is
  * discarded. If the <code>egl.javascript.BigDecimal</code> is out of the possible
 ...
 /**
  * Returns a plain <code>egl.javascript.BigDecimal</code> whose decimal point has
  ...
  * given by:
  * <p><code>
  * this.multiply(TEN.pow(new egl.javascript.BigDecimal(-n)))
  * </code>
  * <p>
  ...
  * @return   A <code>egl.javascript.BigDecimal</code> derived from
  *           <code>this</code>, with the decimal point moved
  *           <code>n</code> places to the left.
  * @stable ICU 2.0
  */
 
 //--public com.ibm.icu.math.egl.javascript.BigDecimal movePointLeft(int n){
 ...
 //--public com.ibm.icu.math.egl.javascript.BigDecimal movePointRight(int n){
 egl.javascript.BigDecimal.prototype.movePointRight = function movePointRight(n) {
  //--com.ibm.icu.math.egl.javascript.BigDecimal res;
  var res;
  res=this.clone(this);
  res.exp=res.exp+n;
 ...
 /**
  * Returns the scale of this <code>egl.javascript.BigDecimal</code>.
  ...
  *         <code>egl.javascript.BigDecimal</code>.
  * @stable ICU 2.0
  */
 ...
 /**
  * Returns a plain <code>egl.javascript.BigDecimal</code> with a given scale.
  * <p>
  ...
  * <code>egl.javascript.BigDecimal</code> then trailing zeros will be added to the
  * decimal part as necessary.
  * <p>
  * If the given scale is less than the length of the decimal part (the
  * scale) of this <code>egl.javascript.BigDecimal</code> then trailing digits
  ...
  * <code>egl.javascript.MathContext.ROUND_UNNECESSARY</code>.
  *
  * @param  scale The <code>int</code> specifying the scale of the
  *               resulting <code>egl.javascript.BigDecimal</code>.
  * @return       A plain <code>egl.javascript.BigDecimal</code> with the given scale.
  * @throws ArithmeticException if <code>scale</code> is negative.
 ...
 //--public com.ibm.icu.math.egl.javascript.BigDecimal setScale(int scale){
  ...
  * <p>
  * If <code>round</code> is <code>egl.javascript.MathContext.ROUND_UNNECESSARY</code>,
  ...
  *               <code>egl.javascript.MathContext.ROUND_UNNECESSARY</code>, and
  *               reducing scale would discard non-zero digits.
  * @stable ICU 2.0
  */
 
 //--public com.ibm.icu.math.egl.javascript.BigDecimal setScale(int scale,int round){
 egl.javascript.BigDecimal.prototype.setScale = function setScale() {
  ...
  if (ourscale==scale)  // already correct scale
   if (this.form==egl.javascript.MathContext.prototype.PLAIN)  // .. and form
    return this;
  ...
  res.form=egl.javascript.MathContext.prototype.PLAIN; // by definition
  return res;
  }
 
 /**
  * Converts this <code>egl.javascript.BigDecimal</code> to a <code>short</code>.
  * If the <code>egl.javascript.BigDecimal</code> has a non-zero decimal part or is
 ...
 /**
  * Returns the sign of this <code>egl.javascript.BigDecimal</code>, as an
  * <code>int</code>.
  * This returns the <i>signum</i> function value that represents the
  * sign of this <code>egl.javascript.BigDecimal</code>.
  * That is, -1 if the <code>egl.javascript.BigDecimal</code> is negative, 0 if it is
  * numerically equal to zero, or 1 if it is positive.
  *
  * @return An <code>int</code> which is -1 if the
  *         <code>egl.javascript.BigDecimal</code> is negative, 0 if it is
  *         numerically equal to zero, or 1 if it is positive.
  * @stable ICU 2.0
  */
 ...
 /**
  * Converts this <code>egl.javascript.BigDecimal</code> to a
  * <code>java.math.egl.javascript.BigDecimal</code>.
  * <p>
  * This is an exact conversion; the result is the same as if the
  * <code>egl.javascript.BigDecimal</code> were formatted as a plain number without
  * any rounding or exponent and then the
  * <code>java.math.egl.javascript.BigDecimal(java.lang.String)</code> constructor
  ...
  * It would not be present in a <code>java.math</code> version.)</i>
  *
  * @return The <code>java.math.egl.javascript.BigDecimal</code> equal in value
  *         to this <code>egl.javascript.BigDecimal</code>.
  * @stable ICU 2.0
  */
 
 //--public java.math.egl.javascript.BigDecimal toegl.javascript.BigDecimal(){
 //-- return new java.math.egl.javascript.BigDecimal(this.unscaledValue(),this.scale());
 //-- }
  ...
  *         to the integer part of this <code>egl.javascript.BigDecimal</code>.
  * @stable ICU 2.0
  */
 
 //--public java.math.BigInteger toBigInteger(){
 //-- com.ibm.icu.math.egl.javascript.BigDecimal res=null;
 //-- int newlen=0;
 //-- byte newmant[]=null;
 //-- {/*select*/
 //-- if ((exp>=0)&(form==com.ibm.icu.math.egl.javascript.MathContext.PLAIN))
 //--  res=this; // can layout simply
 //-- else if (exp>=0)
 //--  {
 //--   res=clone(this); // safe copy
 //--   res.form=(byte)com.ibm.icu.math.egl.javascript.MathContext.PLAIN; // .. and request PLAIN
 //--  }
 //-- else{
 ...
 //--     res.mant=newmant;
 //--     res.form=(byte)com.ibm.icu.math.egl.javascript.MathContext.PLAIN;
 //--     res.exp=0;
 //--    }
 //--  }
 //-- }
 ...
 /**
  * Returns the <code>egl.javascript.BigDecimal</code> as a character array.
 ...
 /**
  * Returns the <code>egl.javascript.BigDecimal</code> as a <code>String</code>.
  * This returns a <code>String</code> that exactly represents this
  * <code>egl.javascript.BigDecimal</code>, as defined in the decimal documentation
  * (see {@link egl.javascript.BigDecimal class header}).
  * <p>
  * By definition, using the {@link #egl.javascript.BigDecimal(String)} constructor
  * on the result <code>String</code> will create a
  * <code>egl.javascript.BigDecimal</code> that is exactly equal to the original
  * <code>egl.javascript.BigDecimal</code>.
  *
  ...
  *         this <code>egl.javascript.BigDecimal</code> multiplied by ten to the
 ...
 /**
  * Translates a <code>double</code> to a <code>egl.javascript.BigDecimal</code>.
  * <p>
  * Returns a <code>egl.javascript.BigDecimal</code> which is the decimal
  * representation of the 64-bit signed binary floating point
  ...
  * {@link #egl.javascript.BigDecimal(java.lang.String)} constructor had then been used.
  * This is typically not an exact conversion.
  *
  * @param  dub The <code>double</code> to be translated.
  * @return     The <code>egl.javascript.BigDecimal</code> equal in value to
  *             <code>dub</code>.
  * @throws NumberFormatException if the parameter is infinite or
  *             not a number.
  * @stable ICU 2.0
 ...
 //--public static com.ibm.icu.math.egl.javascript.BigDecimal valueOf(double dub){
 ...
 //-- return new com.ibm.icu.math.egl.javascript.BigDecimal((new java.lang.Double(dub)).toString());
 //-- }

 /**
  * Translates a <code>long</code> to a <code>egl.javascript.BigDecimal</code>.
  * That is, returns a plain <code>egl.javascript.BigDecimal</code> whose value is
 ...
 //--public static com.ibm.icu.math.egl.javascript.BigDecimal valueOf(long lint){
 //-- return valueOf(lint,0);
 //-- }

 /**
  * Translates a <code>long</code> to a <code>egl.javascript.BigDecimal</code> with a
  * given scale.
  * That is, returns a plain <code>egl.javascript.BigDecimal</code> whose unscaled
  * value is equal to the given <code>long</code>, adjusted by the
  * second parameter, <code>scale</code>.
  * <p>
  * The result is given by:
  * <p><code>
  * (new egl.javascript.BigDecimal(lint)).divide(TEN.pow(new egl.javascript.BigDecimal(scale)))
  * </code>
  * <p>
 ...
 //--public static com.ibm.icu.math.egl.javascript.BigDecimal valueOf(long lint,int scale){
 //-- com.ibm.icu.math.egl.javascript.BigDecimal res=null;
 //-- {/*select*/
 //-- if (lint==0)
 //--  res=ZERO;
 ...
 //--  res=TEN;
 //-- else{
 //--  res=new com.ibm.icu.math.egl.javascript.BigDecimal(lint);
 //-- }
 //-- }
 //-- if (scale==0) 
 //--  return res;
 ...
 /* <sgml> Return char array value of a egl.javascript.BigDecimal (conversion from
       egl.javascript.BigDecimal to laid-out canonical char array).
 ...
 /* <sgml> Checks a egl.javascript.BigDecimal argument to ensure it's a true integer
       in a given range.
 ...
 //--private com.ibm.icu.math.egl.javascript.BigDecimal dodivide(char code,com.ibm.icu.math.egl.javascript.BigDecimal rhs,com.ibm.icu.math.egl.javascript.MathContext set,int scale){
 egl.javascript.BigDecimal.prototype.dodivide = function dodivide(code, rhs, set, scale) {
  //--com.ibm.icu.math.egl.javascript.BigDecimal lhs;
  var lhs;
  //--int reqdig;
  var reqdig;
  ...
  /* Prepare numbers according to egl.javascript.BigDecimal rules */
  reqdig=set.digits; // local copy (heavily used)
  if (reqdig>0) 
   {
    if (lhs.mant.length>reqdig) 
 ...
 egl.javascript.BigDecimal.prototype.extend = function extend(inarr, newlen) {
  //--byte newarr[];
  var newarr;
 ...
 egl.javascript.BigDecimal.prototype.byteaddsub = function byteaddsub(a, avlen, b, bvlen, m, reuse) {
  //--int alength;
  var alength;
  //--int blength;
  var blength;
 ...
 egl.javascript.BigDecimal.prototype.diginit = function diginit() {
  ...
  {op=0;op:for(;op<=(90+99);op++){
   ...
   if (digit>=0) 
    {
     work[op]=(digit%10);
     egl.javascript.BigDecimal.prototype.bytecar[op]=(egl.javascript.BigDecimal.prototype.div(digit,10)); // calculate carry
     continue op;
    }
   // borrowing...
   digit=digit+100; // yes, this is right [consider -50]
   work[op]=(digit%10);
   egl.javascript.BigDecimal.prototype.bytecar[op]=((egl.javascript.BigDecimal.prototype.div(digit,10))-10); // calculate borrow [NB: - after %]
   }
  }/*op*/
  return work;
  }
 ...
 egl.javascript.BigDecimal.prototype.bytedig = egl.javascript.BigDecimal.prototype.diginit();

 /* <sgml> Create a copy of egl.javascript.BigDecimal object for local use.
    <p>This does NOT make a copy of the mantissa array.
    </sgml>
    Arg1 is the egl.javascript.BigDecimal to clone (non-null)
    */
 
 //--private static final com.ibm.icu.math.egl.javascript.BigDecimal clone(com.ibm.icu.math.egl.javascript.BigDecimal dec){
 egl.javascript.BigDecimal.prototype.clone = function clone(dec) {
  //--com.ibm.icu.math.egl.javascript.BigDecimal copy;
  var copy;
  copy=new egl.javascript.BigDecimal();
  copy.ind=dec.ind;
  copy.exp=dec.exp;
  copy.form=dec.form;
  copy.mant=dec.mant;
 ...
 //--private void checkdigits(com.ibm.icu.math.egl.javascript.BigDecimal rhs,int dig){
 egl.javascript.BigDecimal.prototype.checkdigits = function checkdigits(rhs, dig) {
  if (dig==0) 
 ...
 /* <sgml> Round to specified digits, if necessary. </sgml>
    Arg1 is requested egl.javascript.MathContext [with length and rounding mode]
    returns this, for convenience */
 
 //--private com.ibm.icu.math.egl.javascript.BigDecimal round(com.ibm.icu.math.egl.javascript.MathContext set){
 ...
 //private com.ibm.icu.math.egl.javascript.BigDecimal round(int len,int mode){
 ...
 egl.javascript.BigDecimal.prototype.allzero = function allzero(array, start) {
  //--int i=0;
  var i=0;
  if (start<0) 
   start=0;
 ...
 /* <sgml> Carry out final checks and canonicalization
    ...
    Arg1 is requested egl.javascript.MathContext (length to round to, trigger, and FORM)
 ...
 //--private com.ibm.icu.math.egl.javascript.BigDecimal finish(com.ibm.icu.math.egl.javascript.MathContext set,boolean strip){
 egl.javascript.BigDecimal.prototype.finish = function finish(set, strip) {
  //--int d=0;
  var d=0;
  //--int i=0;
  var i=0;
  ...
  this.form=egl.javascript.MathContext.prototype.PLAIN; // preset
